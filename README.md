...
The implemented design follows a layered client-server architecture in which the browser-based p5.js front end owns all latency-sensitive gameplay, while server-side components own identity, persistence, and transaction integrity. At the presentation layer, the game runs as a single-page canvas application with DOM overlays for account and shop interaction. A central front-end controller coordinates rendering, input dispatch, camera updates, map generation, and transitions between menu, authentication, difficulty selection, play, tutorial, pause, map-targeting, shop, defeat, and victory states. This controller sits behind a reverse proxy that serves static assets and forwards API traffic to an Express service. The service layer concentrates low-frequency business logic, including login, email verification, password reset, catalog retrieval, equipment selection, purchase validation, and progress storage. A SQLite database forms the persistence layer, storing users, mail tokens, persistent player progress, and the seeded shop catalog. This separation is well suited to the game’s requirements: movement, collision, aiming, enemy pursuit, and projectile simulation remain responsive in the client, while coins, ownership, and authentication remain authoritative on the server.
The object model is centred on a compact gameplay hierarchy. Vehicle is the shared base class for moving actors and encapsulates position, velocity, acceleration, heading, visual skid effects, and boundary handling. Player and Enemy specialise this behaviour in different ways. Player extends the base kinematics with health, ammunition, coins, selected car, weapon ownership, special-weapon state, and firing logic. Enemy reuses the same movement substrate but adds pursuit and separation behaviours so that police vehicles both chase the player and avoid stacking unnaturally. This inheritance structure is appropriate because both actors are genuinely the same kind of thing at the movement level, but differ in control policy and combat role. Around this core, Projectile models weapon effects with autonomous motion and collision logic, while PowerUp models collectible items such as coins, health, shield, speed boosts, and unlocked special drops. Weapon behaviour is largely configuration-driven, which reduces duplication and allows several attack styles to share one projectile abstraction.
Environmental interaction is represented by Building, which combines map presence, collision bounds, display identity, and interaction affordances. Buildings therefore function as world objects rather than as passive artwork. Their role is reinforced by procedural city generation, which places normal buildings together with a small set of special landmarks, including police stations that act as enemy spawn anchors. However, the implemented interaction model is intentionally simpler than a fully service-specific building system. The hospital and armory are visually distinct and remain meaningful as navigational landmarks, yet both currently route the player into the same shop state rather than into separate hospital and armory workflows. Direct health recovery is handled primarily through collectible health items. This is an important design characteristic because it shows that the environmental layer is currently used more as a trigger mechanism and spatial decision aid than as a collection of deeply differentiated building services.
Three further classes sit at the boundary between gameplay and interface. AuthUI manages login, registration, verification, token storage, and progress synchronisation. ShopUI manages catalog loading, tabbed presentation, selection, purchase requests, and application of returned server progress to the live player object. TutorialSystem observes gameplay context and temporarily interrupts the main loop with onboarding overlays when enemies, pickups, or special weapons first appear. These classes are not part of the combat domain itself; instead, they mediate between user actions, persistent state, and the front-end controller. This distinction between domain objects and boundary/control objects keeps responsibilities clear: combat classes simulate the world, while UI classes negotiate with the player and the backend.
Behaviourally, the system is organised around explicit state transitions and message passing across layers. The login path begins when the player chooses to start or access the shop, after which the client checks service availability, presents the authentication overlay, submits credentials, stores the returned token, and then requests persistent progress before enabling gameplay or menu shopping. The purchase path follows the same pattern of controlled collaboration: the front end opens the shop, loads the server catalog, submits an authenticated purchase or selection request, and then applies the returned progress snapshot to the active player state. On the server, JWT verification guards each protected request, database reads reconstruct the player’s current balance and ownership, and validated writes return a fresh progress view to the client. This produces a clear separation between immediate play-state changes on the client and durable economic updates on the server. Difficulty selection adds a further behavioural branch by altering survival time and tuning parameters, with the implemented modes yielding 60, 90, and 120 second targets and a harder configuration that reduces player speed while increasing enemy speed.

...
### Implementation
---
The core of our game was implemented using a split client-server architecture. The front-end, entirely powered by the **p5.js** library, is responsible for the core game loop, canvas rendering, physics calculations, and user input handling. The back-end, built with **Node.js, Express, and SQLite**, handles user authentication (using secure session management), persistent player progress, and shop inventories. We separated the implementation into discrete, iterative phases: establishing basic vehicle movement, implementing map boundaries and collisions, adding enemy AI and combat mechanics, and finally integrating the backend-driven shop and progression systems. The communication between client and server is handled via asynchronous REST API calls, ensuring the game loop remains uninterrupted during data syncs.

#### Technical Challenge 1: Physics Engine, Collision Logic, and Weapon Mechanics
Implementing smooth, responsive, yet weighty vehicle mechanics was our primary technical hurdle. The movement logic required a robust simulation of momentum, acceleration, and handling. Initially, cars felt either too "floaty" or unnaturally rigid. To solve this, we implemented custom physics functions utilizing 2D vectors for velocity, acceleration, and friction. We introduced dampening forces and angular velocity calculations to ensure driving felt engaging, especially when turning at high speeds. Additionally, we implemented dynamic drift trajectory effects. This required storing a localized queue of the vehicle's historical coordinates and drawing them with fading alpha values behind the car. Managing this history array—constantly shifting old positions out to cap its size—added a layer of visual complexity to the canvas rendering without dropping the crucial 60 FPS frame rate.

Tied intimately to movement was the collision logic. Developing a robust, performant collision system between the player, swarming enemies, map boundaries, and interactive buildings required precise coordinate mapping. We utilized Axis-Aligned Bounding Box (AABB) algorithms and circular hitboxes depending on the entity type. Early on, players could clip through walls at high speeds, or enemies would stack perfectly on top of one another, forming an indistinguishable mass. We resolved the clipping issue by implementing predictive continuous boundary checks rather than reactive ones. To fix the enemy stacking, we implemented a separation steering behavior—inspired by classic boids flocking algorithms—which calculates a repulsive force between proximate enemy instances, forcing them to fan out naturally as they chase the player.

Combat implementation introduced further complexity. We had to carefully program weapon flight speeds, projectile lifetimes, and varying damage values. Because projectiles were constantly being fired, we faced memory management issues. A notable bug arose where a missile flying indefinitely out of the map boundaries locked player control and caused memory leaks; we resolved this by implementing strict garbage collection for out-of-bounds projectiles, purging them from the update array once their coordinates exceeded the map dimensions. Furthermore, ensuring that different weapons had distinct visual effects (e.g., explosive area-of-effect radius vs. rapid-fire linear lasers) required creating a modular `Projectile` class that dynamically loaded different sprite sheets, hitboxes, and particle emission logic upon impact.

#### Technical Challenge 2: Asset Loading, State Management, and Game Balancing
Because our game features numerous rich visual and audio assets, users experienced significant delays, "pop-in," and broken canvas renders during their first play session when the browser cache was empty. To resolve this, we strictly optimized our asset management pipeline. We utilized `p5.preload()` to enforce the asynchronous preloading of all heavy assets (sprites, background music, sound effects, and UI components). We paired this with a custom visual loading screen that listens to the load progress, ensuring that the `setup()` function and subsequent game loop only initiated once all resources were fully verified and loaded into memory.

Another significant architectural challenge within this area was global state management—specifically, unintended bugs caused by the interaction of the continuous survival timer and transactional shop mechanics. Initially, the global `deltaTime` and survival timer would continue ticking while the player was browsing the weapon shop overlay, leading to unfair, off-screen game-overs. We had to restructure the front-end controller using a State Machine pattern. We separated logic into discrete state classes (e.g., `PlayState`, `ShopState`, `PauseState`). When the player enters a building, the controller pushes the `ShopState` onto an active state stack, which strictly halts the game logic updates and delta-time progression for the underlying `PlayState`, while still calling its `render()` function to maintain the visual continuity of the background environment. 

Finally, we struggled with game balancing, spawn algorithms, and user experience. Players' subjective expectations often conflicted with our initial, somewhat brutal system design. Creating an intuitive experience for first-time players without relying heavily on text-heavy tutorials required extensive iterative tuning. We spent significant time refining the spawn algorithms for randomly generated map items and enemy waves. Rather than purely random spawns, we implemented a time-based scaling difficulty curve that gradually increases enemy density and speed as the timer progresses, while periodically guaranteeing health and shield drops based on the player's current health pool and distance. This dynamic adjustment ensured the game felt consistently challenging but inherently fair across repeated sessions.
